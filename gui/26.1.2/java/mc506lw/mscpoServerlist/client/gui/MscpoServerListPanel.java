...
public class MscpoServerListPanel extends AbstractContainerWidget {
	...
	public MscpoServerListPanel(JoinMultiplayerScreen screen, Minecraft client) {
		super(0, 0, 300, 200, Component.translatable("mscpo.title"), AbstractScrollArea.defaultSettings(20));
		this.screen = screen;
		this.client = client;
	...
	protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float deltaTicks) {
		...
		context.text(this.client.font, Component.translatable("mscpo.title"), contentX, y + 1, -1);
		...
		if (this.categoryDropdown.isOpen()) {
			this.categoryDropdown.extractRenderState(context, mouseX, mouseY, deltaTicks);
		}
		if (this.subCategoryDropdown.isOpen()) {
			this.subCategoryDropdown.extractRenderState(context, mouseX, mouseY, deltaTicks);
		}
		if (this.drawerMode) {
	...
	private void drawTabStrip(GuiGraphicsExtractor context, int x, int y, int w, int h, String label) {
		context.fill(x, y, x + DRAWER_TAB_WIDTH, y + h, 0x99101010);
		context.fill(x + DRAWER_TAB_WIDTH - 1, y, x + DRAWER_TAB_WIDTH, y + h, 0xFF3C3C3C);
		context.centeredText(this.client.font, label, x + DRAWER_TAB_WIDTH / 2, y + h / 2 - 4, -1);
	}
